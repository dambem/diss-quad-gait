...
found = False
gradients = np.gradient(oscillator_values[0, 5000:])
# print(gradients)
# print(np.shape(gradients))
count = 0
first = 0
second = 0
for n, val in np.ndenumerate(gradients):
    n= n[0]

    if (n < 2 ):
        continue;
    else:
        if(gradients[n] < 0 and gradients[n-1] > 0):
            if (found == False):
                # print(time_array[0, n])
                first = time_array[0,n+5000]
                found = True
            elif (found == True):
                second = time_array[0, n+5000]
                found = False
                print(second-first)
                # break;
